...
class AppProvider extends ChangeNotifier {
  ...
  bool get bypassTranslation => _bypassTranslation;

  String get speechText => _speechText;
  String get translatedText => _translatedText;
  String get geminiResponse => _geminiResponse;
  String get translatedResponse => _translatedResponse;
  String get logs => _logs;
  List<ChatMessage> get messages => _messages;

  AppProvider() {
    _addLog('AppProvider initialized');
    _initSpeech();
    _initTts();
    _checkConnectivity();
  ...
  Future<void> _initSpeech() async {
    ...
    bool available = await _speechToText.initialize(
      onError: (error) {
        _addLog('Speech recognition error: $error');
      ...
      onStatus: (status) => _addLog('Speech status: $status'),
      debugLogging: true, // Enable detailed logging
    );

    if (available) {
      _addLog('Speech recognition initialized successfully');
      ...
      final marathiLocales =
          locales
              .where(
                (locale) =>
                    locale.localeId.contains('mr') ||
                    locale.name.toLowerCase().contains('marathi') ||
                    locale.localeId.contains(
                      'in',
                    ) || // Also check for Indian locales as fallback
                    locale.name.toLowerCase().contains('india'),
              )
              .toList();
      ...
      _addLog('Speech recognition failed to initialize');
    }
  }

  // Initialize text-to-speech
  Future<void> _initTts() async {
    _addLog('Initializing text-to-speech...');

    _flutterTts.setErrorHandler((error) {
      _addLog('TTS error: $error');
    });

    _flutterTts.setStartHandler(() {
      _addLog('TTS started speaking');
    });

    _flutterTts.setCompletionHandler(() {
      _addLog('TTS finished speaking');
      _isReplying = false;
      notifyListeners();
    });
    ...
    _addLog('Available TTS languages: ${languages.toString()}');

    // Check if Marathi is available
    final hasMarathi =
        languages.toString().toLowerCase().contains('marathi') ||
        languages.toString().contains('mr');
    _addLog('Marathi TTS ${hasMarathi ? 'is' : 'is not'} available');

    if (hasMarathi) {
      await _flutterTts.setLanguage('mr-IN'); // Marathi language
    } else {
      // Fallback to Hindi or English
      if (languages.toString().toLowerCase().contains('hindi') ||
          languages.toString().contains('hi')) {
        await _flutterTts.setLanguage('hi-IN'); // Hindi language
        _addLog('Fallback to Hindi TTS');
      } else {
        await _flutterTts.setLanguage('en-IN'); // Indian English
        _addLog('Fallback to Indian English TTS');
      }
    }
  ...
  Future<void> _checkConnectivity() async {
    _addLog('Checking internet connectivity...');
  ...
  void _setupConnectivityListener() {
    _addLog('Setting up connectivity listener...');
  ...
  Future<void> startListening() async {
    ...
    if (!_isConnected) {
      _addLog('Cannot start listening: No internet connection');
      return;
    }
    ...
    try {
      ...
      await _speechToText.listen(
        onResult: (result) {
          if (result.recognizedWords.isNotEmpty) {
            ...
            _addLog('Recognized words: $_speechText');
          } else {
            _addLog('No words recognized in this result');
          }
          notifyListeners();
        },
            ...
            _silenceTimer ??= Future.delayed(
              ...
              () {
                if (_isListening && _speechText.isNotEmpty) {
                  _addLog('Auto-stopping due to silence');
                  stopListening();
                }
                _silenceTimer = null;
              },
      ...
      _addLog('Listening started successfully with default locale');
    } catch (e) {
      _addLog('Error starting listening: $e');
      _isListening = false;
      _speechText = 'Error starting speech recognition. Please try again.';
      notifyListeners();
    }
  }
  ...
  Future<void> stopListening() async {
    _addLog('Stopping speech recognition...');
    try {
      ...
      if (_speechText.isNotEmpty) {
        _addLog('Processing voice input: "$_speechText"');
        processVoiceInput();
      } else {
        _addLog('No speech detected');
      }
    } catch (e) {
      _addLog('Error stopping speech recognition: $e');
    }
  ...
  Future<void> processVoiceInput() async {
    if (!_isConnected) {
      _addLog('Cannot process: No internet connection');
      return;
    }

    if (_speechText.isEmpty) {
      _addLog('Cannot process: Empty speech text');
      return;
    }
    ...
    try {
      _addLog('Starting processing workflow...');

      // Add user message to the conversation
      _addMessage(_speechText, true, _bypassTranslation, false);

      if (_bypassTranslation) {
        // Direct mode: Skip translation and send directly to Gemini
        _addLog('Using direct mode (bypassing translation)');
        _translatedText = _speechText; // Use original text

        // Get response from Gemini
        _addLog('Sending untranslated text directly to Gemini');
        await getGeminiResponse();

        // No translation back needed
        _translatedResponse = _geminiResponse;
        _addLog('Response received directly from Gemini');

        // Add AI response to the conversation
        _addMessage(_translatedResponse, false, _bypassTranslation, false);

        // Speak response
        _addLog('Speaking response');
        speakResponse();
      } else {
        // Standard mode with translation
        _addLog('Using standard mode with translation');

        // 1. Translate Marathi to English
        _addLog('Step 1: Translating from Marathi to English');
        await translateToEnglish(_speechText);

        // 2. Get response from Gemini
        _addLog('Step 2: Getting response from Gemini');
        await getGeminiResponse();

        // 3. Translate English response to Marathi
        _addLog('Step 3: Translating response from English to Marathi');
        await translateToMarathi(_geminiResponse);

        // 4. Add AI response to the conversation
        _addMessage(_translatedResponse, false, _bypassTranslation, false);

        // 5. Speak response
        _addLog('Step 4: Speaking response');
        speakResponse();
      }

      _addLog('Processing completed successfully');
    } catch (e) {
      _addLog('Error during processing workflow: $e');
      _addMessage(
  ...
  Future<void> translateToEnglish(String text) async {
    _addLog('Translating from Marathi to English: "$text"');
    try {
      ...
      _addLog('Translation result: "$_translatedText"');
      notifyListeners();
    } catch (e) {
      _addLog('Translation error: $e');
      _translatedText = text; // Fallback to original text
      _addLog('Using original text as fallback');
      notifyListeners();
    }
  }
  ...
  Future<void> getGeminiResponse() async {
    _addLog('Sending request to Gemini: "$_translatedText"');
    try {
      final apiKey = dotenv.env['GEMINI_API_KEY'] ?? '';
      if (apiKey.isEmpty) {
        _addLog('Error: Gemini API key is missing');
        _geminiResponse =
      ...
      _addLog('Initializing Gemini model...');
      ...
      for (final modelName in modelOptions) {
        try {
          _addLog('Trying model: $modelName');
...
""";
          ...
          _addLog('Sending content to model: $modelName');
          final content = [Content.text(enhancedText)];
          final response = await model.generateContent(content);

          _geminiResponse = response.text ?? 'No response from Gemini';
          // Apply post-processing to ensure markdown formatting
          _geminiResponse = _ensureMarkdownFormatting(_geminiResponse);
          _addLog(
          ...
          _addLog('Error with model $modelName: $lastErrorMessage');
      ...
      _addLog('Gemini error: $errorMessage');
      ...
      if (errorMessage.contains('API key')) {
        ...
        _geminiResponse = 'Error: API quota exceeded. Please try again later.';
  ...
  String _formatErrorMessage(dynamic error) {
    ...
    if (message.contains('ApiException:')) {
      return message.split('ApiException:').last.trim();
    }
    return message;
  }

  // Translate from English to Marathi
  Future<void> translateToMarathi(String text) async {
    _addLog('Translating from English to Marathi: "$text"');
    try {
      final translation = await _translator.translate(
        text,
        from: 'en', // English
      ...
      _addLog('Translation result: "$_translatedResponse"');
      notifyListeners();
    } catch (e) {
      _addLog('Translation error: $e');
      _translatedResponse = text; // Fallback to original text
      _addLog('Using original text as fallback');
      notifyListeners();
    }
  }
  ...
  Future<void> speakResponse() async {
    if (!_isConnected) {
      _addLog('Cannot speak: No internet connection');
      return;
    }

    if (_translatedResponse.isEmpty) {
      _addLog('Cannot speak: Empty response');
      return;
    }

    // If already speaking, stop instead
    if (_isReplying) {
      _addLog('Stopping speech');
    ...
    try {
      ...
      String cleanedText = _cleanTextForTTS(_translatedResponse);

      _addLog('Speaking cleaned response: "$cleanedText"');
  ...
  void copyResponse() {
    _addLog('Copied response to clipboard: "$_translatedResponse"');
  ...
  Future<void> processTextInput(String text) async {
    ...
    if (text.isEmpty) {
      _addLog('Cannot process: Empty text input');
      return;
    }

    _speechText = text;
    _addLog('Processing direct text input: "$_speechText"');
  ...
  void toggleBypassTranslation() {
    _bypassTranslation = !_bypassTranslation;
    _addLog(
  ...
  void _addMessage(String text, bool isUser, bool isDirectMode, bool isError) {
    ...
    if (isUser && !isError) {
      Future.delayed(Duration(milliseconds: 500), () {
        analyzeSentiment(text, messageIndex);
      });
    }
  }

  // Analyze sentiment of a message
  Future<void> analyzeSentiment(String text, int messageIndex) async {
    // Don't analyze very short messages or if we're offline
    if (text.length < 3 ||
        messageIndex < 0 ||
        messageIndex >= _messages.length ||
        !_isConnected) {
      _addLog('Cannot analyze sentiment: Invalid message or no connection');
      return;
    }

    // Prevent multiple simultaneous sentiment analyses
    if (_isAnalyzingSentiment) {
      _addLog('Skipping sentiment analysis: Already analyzing another message');
      return;
    }
    ...
    try {
      ...
      _addLog('Analyzing sentiment for message: "$text"');
...
""";
      ...
      String sentiment = (response.text ?? 'neutral').trim().toLowerCase();
      _addLog('Sentiment detected: $sentiment');
      ...
      _addLog('Error analyzing sentiment: ${_formatErrorMessage(e)}');
    } finally {
  ...
  void clearConversation() {
    ...
    _geminiResponse = '';
    _translatedResponse = '';
    _addLog('Conversation history cleared');
    notifyListeners();
  }

  // Edit and reprocess a message
  Future<void> editAndReprocessMessage(
    String originalMessage,
    String editedMessage,
  ) async {
    _addLog('Editing message: "$originalMessage" -> "$editedMessage"');

    // Find the original message in history
    int messageIndex = _messages.indexWhere(
      (msg) => msg.isUser && msg.text == originalMessage,
    );

    if (messageIndex == -1) {
      _addLog('Original message not found in history');
      return;
    }

    // Update the message text
    final originalDirectMode = _messages[messageIndex].isDirectMode;
    _messages[messageIndex] = ChatMessage(
      text: editedMessage,
      isUser: true,
      isDirectMode: originalDirectMode,
      isError: false,
      isEdited: true,
    );

    // Remove all messages that come after this one
    if (_messages.length > messageIndex + 1) {
    ...
    Future.delayed(Duration(milliseconds: 500), () {
      analyzeSentiment(editedMessage, messageIndex);
    });

    // Reprocess the edited message
    _speechText = editedMessage;
    _addLog('Reprocessing edited message: "$editedMessage"');
  ...
  Future<String> summarizeConversation() async {
    _addLog('Summarizing conversation...');

    if (_messages.isEmpty) {
      _addLog('No messages to summarize');
      return "No conversation to summarize yet.";
    }

    if (!_isConnected) {
      _addLog('Cannot summarize: No internet connection');
...
""";
      ...
      _addLog('Sending summarization request to Gemini');

      final apiKey = dotenv.env['GEMINI_API_KEY'] ?? '';
      if (apiKey.isEmpty) {
        _addLog('Error: Gemini API key is missing');
        _isProcessing = wasProcessing; // Restore previous state
      ...
      final summary = response.text ?? 'Could not generate summary';
      _addLog('Summary generated: "$summary"');

      // Ensure summary has markdown formatting
      return _ensureMarkdownFormatting(summary);
    } catch (e) {
      final errorMessage = _formatErrorMessage(e);
      _addLog('Summarization error: $errorMessage');
